...
const MunchiesReviewsScreen = ({ route }) => {
    const { munchiesId, munchiesName } = route.params; // Get the munchies id and name passed from the previous screen.
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const response = await axios.get(`${url}/munchies/${munchiesId}`);
        ...
        >
            <ScrollView contentContainerStyle={{ paddingHorizontal: 0 }}>
                ...
                <View style={styles.container}>
                    <Text style={styles.title}>Reviews for {munchiesName}{renderAverageStars()}</Text>
...
export default MunchiesReviewsScreen;
